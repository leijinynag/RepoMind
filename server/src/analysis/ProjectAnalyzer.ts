...
export class ProjectAnalyzer {
  ...
  private async parsePackageJson(repoPath: string) {
    ...
    const packageJson = await fs.readFile(
      path.join(repoPath, packageJsonFile),
      "utf-8",
    );
...
`;
  ...
  async analyze(repoId: string) {
    ...
    // 格式化外部依赖
    const externalDeps = Object.entries(packageJson.dependencies || {}).map(
      ([name, version]) => ({
        name,
    ...
    const memory = await CodebaseMemory.findOneAndUpdate(
      { repoId },
      memoryData,
